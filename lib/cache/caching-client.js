...
var requestsQue = []
var currentOpenRequestsCount = 0
...
CachingRegistryClient.prototype._invalidatingRequest = function (uri, params, cb) {
  ...
  this._request(uri, params, function () {
    ...
    function checkContinue () {
      if (requestsQue.length) {
        performRequest()
      } else {
        currentOpenRequestsCount--
      }
    }

    var requestLimit = npm.config.get('conn-limit')
    function performRequest () {
      log.silly('request', 'performing request with',
        currentOpenRequestsCount, 'of', requestLimit, 'requests open')
      var tmp = requestsQue.shift()
      checkContinue()
      tmp.cb.apply(client, tmp.args)
    }

    log.silly('request', 'check', currentOpenRequestsCount, '<', requestLimit,
      ' ||', requestLimit, ' == null')
    if (requestLimit === null || currentOpenRequestsCount < requestLimit) {
      currentOpenRequestsCount++
      performRequest()
    } else {
      log.silly('request', 'rate limit reached, awaiting others to finish before continueing')
    }
    // else current requests will recursively call it later when it shifts off of requestsQue
  })
}
